...
public class BeatDashDbContext(DbContextOptions<BeatDashDbContext> options) : IdentityDbContext<User, IdentityRole<Guid>, Guid>(options) {
    ...
    protected override void OnModelCreating(ModelBuilder builder) {
        ...
        builder.Entity<SensorSample>(c => {
            c.HasKey(x => x.Id);
            // Time-window lookups per user+metric; unique so re-pushed samples dedupe.
            c.HasIndex(x => new { x.UserId, x.Metric, x.RecordedAt }).IsUnique();
            c.HasOne(x => x.User)
                .WithMany()
                .HasForeignKey(x => x.UserId)
                .OnDelete(DeleteBehavior.Cascade);
