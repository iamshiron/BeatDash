...
public sealed class BeatmapAnalysisService(
        ...
        try {
            ...
            foreach (var difficulty in beatmap.Difficulties) {
                ...
                if (parsed is null) {
                    ...
                    Upsert(db, difficulty, BeatmapAnalysisStatus.DifficultyNotFound, level: null, parsed: null);
                    continue;
                }

                Upsert(db, difficulty, BeatmapAnalysisStatus.Success, level, parsed);
                analyzed++;
            }
    ...
    private async Task RecordAllAsync(
        ...
        foreach (var difficulty in beatmap.Difficulties) {
            Upsert(db, difficulty, status, level: null, parsed: null);
        }
        await db.SaveChangesAsync(ct);
    }
    ...
    private static void Upsert(
        BeatDashDbContext db, BeatmapDifficulty difficulty,
        BeatmapAnalysisStatus status, ParsedLevel? level, ParsedBeatmap? parsed) {
